...
<body>
    ...
    <main>
        ...
        <?php
            ...
            $consultaSQL_ph = "SELECT SUM(estado) as porhoras FROM parqueo 
            ...
            $consultaSQL_m = "SELECT SUM(estado) as mensualidad FROM parqueo 
            INNER JOIN vehiculoparqueo ON vehiculoparqueo.idParqueo_vp = parqueo.idParqueo 
            INNER JOIN vehiculo ON vehiculo.idVehiculo = vehiculoparqueo.idVehiculo_vp 
            WHERE tipoServicio='Mensualidad'";
            $resultado = $transaccion->buscarBD($consultaSQL_ph);
            $ocupadosPorHoras = $resultado[0]["porhoras"];
            $resultado = $transaccion->buscarBD($consultaSQL_m);
            $ocupadosMensualidad = $resultado[0]["mensualidad"];
